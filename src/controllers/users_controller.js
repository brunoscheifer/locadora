...
export const signup = async (req,res) => {
    try {
        ...
        const token = jwtService.gerarToken({
            tipo: user.permission_type,
            email: user.email,
            _id: user._id
        })
...
export const login = async (req,res) => {
    try {
        ...
        if(user && (await user.senha.senhaCorreta(req.body.senha))) {
            const token = jwtService.gerarToken({
                tipo: user.permission_type,
                email: user.email,
                _id: user._id
        ...
        res.json({token})
            ...
            res.status(404).json('Email ou senha invalidos')
        }
    } catch (error) {
        res.status(400).json(error)
    }
